...
// Sends a cancellation notice to a registered team when the tournament
// they registered for has been cancelled or deleted by the admin. If the
// team had already paid their entry fee, this also tells them a refund
// is on its way; if not, it simply confirms no payment is due.
export async function sendTournamentCancellationEmail(team, tournament) {
const fromAddress = process.env.SMTP_FROM || process.env.SMTP_USER;
const t = getTransporter();

const hasRefund = team.payment_status === 'paid' && Number(team.amount_paid || 0) > 0;
const subject = 'Tournament Cancelled - ' + tournament.name + ' | PlayBox Kashmir';
const captainName = escapeHtml(team.captain_name || 'there');
const firstName = captainName.split(' ')[0];
const amountPaidDisplay = 'Rs ' + Number(team.amount_paid || 0).toFixed(2);

const statusBadgeColor = hasRefund ? '#15803D' : '#B91C1C';
const statusBadgeBg = hasRefund ? '#DCFCE7' : '#FEE2E2';
const statusBadgeText = hasRefund ? 'Refund Approved' : 'No Payment Due';
...
const rightColumn = [
detailBlock('Format', escapeHtml(tournament.format)),
detailBlock('Start Date', formatDateNice(tournament.start_date))
].join('');

const refundHtml = hasRefund
? ('<p style="margin:0 0 4px;font-size:14px;color:#27272a;">Amount Paid: <strong>' + amountPaidDisplay + '</strong></p>' +
'<p style="margin:12px 0 0;font-size:13px;color:#52525b;line-height:1.5;">Your refund will be processed to the original payment method within 5-7 business days.</p>')
: ('<p style="margin:0;font-size:13px;color:#52525b;line-height:1.5;">You had not completed payment for this tournament, so no refund is due.</p>');
...
'<meta charset="UTF-8" />',
'<meta name="viewport" content="width=device-width, initial-scale=1.0" />',
'<title>Tournament Cancelled - PlayBox Kashmir\u2122</title>',
'</head>',
'<body style="margin:0;padding:0;background-color:#f4f4f5;">',
'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f4f5;padding:24px 0;font-family:\'Segoe UI\',Arial,Helvetica,sans-serif;">',
'<tr><td align="center">',
'<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(185,28,28,0.12);">',
'<tr><td align="center" style="background-color:#B91C1C;padding:32px 24px;">',
'<img src="' + LOGO_URL + '" alt="PlayBox Kashmir" width="64" height="64" style="display:block;margin:0 auto 12px;border-radius:12px;" />',
'<h1 style="margin:0;color:#ffffff;font-size:22px;">Tournament Cancelled</h1>',
'</td></tr>',
'<tr><td style="padding:32px 24px;">',
'<p style="margin:0 0 16px;font-size:15px;color:#27272a;">Hi ' + firstName + ',</p>',
'<p style="margin:0 0 24px;font-size:15px;color:#27272a;line-height:1.6;">We are sorry to let you know that <strong>' + escapeHtml(tournament.name) + '</strong> has been cancelled, and your team\'s entry is no longer active.</p>',
'<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>',
'<td valign="top" width="50%" style="padding-right:12px;">' + leftColumn + '</td>',
'<td valign="top" width="50%" style="padding-left:12px;">' + rightColumn + '</td>',
'</tr></table>',
...
refundHtml,
'</div>',
'<p style="margin:24px 0 0;font-size:13px;color:#71717a;line-height:1.6;">If you have any questions about this cancellation' + (hasRefund ? ' or your refund' : '') + ', please contact us at <a href="mailto:' + SUPPORT_EMAIL + '" style="color:#0F766E;">' + SUPPORT_EMAIL + '</a>.</p>',
'</td></tr>',
'<tr><td style="background-color:#fafafa;padding:24px;text-align:center;border-top:1px solid #e4e4e7;">',
'<img src="' + LOGO_URL + '" alt="PlayBox Kashmir" width="32" height="32" style="display:block;margin:0 auto 8px;border-radius:8px;" />',
'<p style="margin:0 0 4px;font-size:13px;color:#71717a;">PlayBox Kashmir\u2122</p>',
'<p style="margin:12px 0 0;font-size:12px;color:#a1a1aa;">The PlayBox Kashmir\u2122 Team</p>',
'</td></tr>',
...
// Sends the automated daily "Today's Booking Sheet" report (lib/dailyReport.js)
// as a PDF attachment. Always sent from contact@playboxkashmir.com (the
// verified Brevo sender used for all booking emails), regardless of what
// SMTP_FROM is currently configured to, since this report is specifically
// requested to come from that address.
export async function sendDailyReportEmail({ to, subject, pdfBuffer, pdfFilename }) {
  const t = getTransporter();
  const html = [
    '<!DOCTYPE html>',
    '<html lang="en">',
    ...
    '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f3f4f6;padding:32px 12px;font-family:\'Segoe UI\',Arial,Helvetica,sans-serif;">',
    '<tr><td align="center">',
    '<table role="presentation" width="520" cellpadding="0" cellspacing="0" style="width:520px;max-width:520px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(15,118,110,0.12);">',
    '<tr><td align="center" style="background-color:#0F766E;padding:28px 24px;">',
    '<div style="font-size:19px;line-height:1.3;font-weight:700;color:#ffffff;">' + escapeHtml(subject) + '</div>',
    '</td></tr>',
    '<tr><td style="padding:28px 32px;">',
    '<p style="margin:0;font-size:14px;line-height:1.6;color:#374151;">The booking sheet for the day is attached as a PDF - it lists every booking on the books, any tournament matches scheduled, and the remaining open slots per facility, ready to print for the front desk.</p>',
    '</td></tr>',
    '<tr><td style="padding:0 32px 28px;"><p style="margin:0;font-size:12px;color:#9ca3af;">Automated daily report - PlayBox Kashmir™</p></td></tr>',
    '</table>',
    '</td></tr>',
    '</table>',
    '</body>',
  ...
  await t.sendMail({
    from: 'PlayBox Kashmir <contact@playboxkashmir.com>',
    to,
    subject,
    html,
    attachments: [
...
const MEMBERSHIP_TERMS_URL = 'https://register.playboxkashmir.com/membership-terms.html';
const ADMIN_NOTIFY_EMAIL = 'playboxkashmir@gmail.com';

// Sent once Razorpay confirms payment for a public membership sign-up on
// register.playboxkashmir.com (see api/razorpay-webhook.js) - by the time
// this goes out the membership already exists in the database with
// status 'active', so this is a genuine welcome/receipt email, not a
// "we'll be in touch" placeholder.
export async function sendMembershipActivatedEmail(membership, plan) {
  const fromAddress = process.env.SMTP_FROM || process.env.SMTP_USER;
  const t = getTransporter();

  const subject = 'Membership Activated - ' + plan.name + ' | PlayBox Kashmir';
  const memberName = escapeHtml(membership.member_name || 'there');
  const firstName = memberName.split(' ')[0];
  const amountPaid = Number(membership.amount_paid || plan.price || 0);
  const termsAcceptedLabel = membership.terms_accepted_at ? formatDateNice(membership.terms_accepted_at) : formatDateNice(new Date());

  const leftColumn = [
    detailBlock('Plan', escapeHtml(plan.name)),
    detailBlock('Amount Paid', '&#8377;' + escapeHtml(amountPaid) + ' (' + billingCycleLabel(plan.billing_cycle).toLowerCase() + ')'),
    detailBlock('Valid Until', formatDateNice(membership.end_date)),
    plan.complimentary_slots > 0 ? detailBlock('Complimentary Slots', escapeHtml(plan.complimentary_slots) + ' per ' + billingCycleLabel(plan.complimentary_frequency).toLowerCase()) : ''
  ].join('');

  const rightColumn = [
    detailBlock('Name', memberName),
    detailBlock('Registered Email', escapeHtml(membership.member_email)),
    detailBlock('Phone', escapeHtml(membership.member_phone)),
    detailBlock('Status', '<span style="display:inline-block;background:#e6f4ea;color:#15803d;font-size:13px;font-weight:700;padding:5px 14px;border-radius:999px;">Active</span>')
  ].join('');
  ...
  const html = [
    ...
    '<meta charset="UTF-8" />',
    '<meta name="viewport" content="width=device-width, initial-scale=1.0" />',
    '<title>Membership Activated - PlayBox Kashmir™</title>',
    '<style>',
    "  body, table, td { font-family: 'Segoe UI', Arial, Helvetica, sans-serif; }",
    '  @media only screen and (max-width: 480px) {',
    '    .pk-container { width: 100% !important; }',
    ...
    '            <td align="center" style="background-color:#0F766E;padding:36px 24px 30px;">',
    '              <img src="' + LOGO_URL + '" alt="PlayBox Kashmir™" width="64" height="64" style="display:block;margin:0 auto 14px;border-radius:14px;" />',
    '            <div style="font-size:22px;line-height:1.3;font-weight:700;color:#ffffff;">Membership Activated! &#127881;</div>',
    '            </td>',
    '          </tr>',
    ...
    '            <td class="pk-body-pad" style="padding:32px 40px 8px;">',
    '              <p style="margin:0 0 6px;font-size:16px;color:#111827;">Hey ' + firstName + ',</p>',
    '            <p style="margin:0;font-size:14px;line-height:1.6;color:#4b5563;">Payment received - your <strong>' + escapeHtml(plan.name) + '</strong> membership at <strong>PlayBox Kashmir™</strong> is now active. Here are your details:</p>',
    '            </td>',
    '          </tr>',
    ...
    '          <tr>',
    '            <td class="pk-body-pad" style="padding:12px 40px 0;">',
    '              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#eff6ff;border-left:4px solid #3b82f6;border-radius:8px;">',
    '                <tr>',
    '                  <td style="padding:18px 20px;">',
    '                    <div style="font-size:14px;font-weight:700;color:#1e3a8a;margin-bottom:6px;">Using your membership</div>',
    '                    <p style="margin:0 0 8px;color:#1e3a8a;font-size:13.5px;line-height:1.7;">Discounts and complimentary slots are tied to the registered email above - you must book with this email and be present in person to redeem them. Your plan is valid for one year from today and is non-refundable and non-transferable.</p>',
    '                    <p style="margin:0;color:#1e3a8a;font-size:13.5px;line-height:1.7;">You accepted our <a href="' + MEMBERSHIP_TERMS_URL + '" style="color:#0F766E;font-weight:600;">Membership Terms and Conditions</a> on ' + termsAcceptedLabel + ' when completing this payment. Please keep a copy for your records.</p>',
    '                  </td>',
    '                </tr>',
    '              </table>',
    '            </td>',
    ...
    '          <tr>',
    '            <td class="pk-body-pad" style="padding:28px 40px 0;">',
    '              <p style="margin:0;font-size:12.5px;line-height:1.7;color:#6b7280;text-align:center;">',
    '                Questions about your membership? Email us at <a href="mailto:' + SUPPORT_EMAIL + '" style="color:#0F766E;font-weight:600;text-decoration:none;">' + SUPPORT_EMAIL + '</a>.',
    '              </p>',
    '            </td>',
    '          </tr>',
...
// Sent alongside sendMembershipActivatedEmail above, but to the business
// inbox instead of the member - lets staff know a new paying member has
// come in through the public register.playboxkashmir.com sign-up flow,
// without having to check the admin dashboard, and includes the Razorpay
// identifiers needed to look the payment up if something needs checking.
export async function sendMembershipAdminNotificationEmail(membership, plan) {
  const fromAddress = process.env.SMTP_FROM || process.env.SMTP_USER;
  const t = getTransporter();

  const subject = 'New Membership Payment - ' + plan.name + ' - ' + (membership.member_name || 'Member');
  const memberName = escapeHtml(membership.member_name || '');
  const amountPaid = Number(membership.amount_paid || plan.price || 0);
  const termsAcceptedLabel = membership.terms_accepted_at ? formatDateNice(membership.terms_accepted_at) : 'Not recorded';

  const leftColumn = [
    detailBlock('Member Name', memberName || '-'),
    detailBlock('Email', escapeHtml(membership.member_email)),
    detailBlock('Phone', escapeHtml(membership.member_phone)),
    membership.notes ? detailBlock('Notes', escapeHtml(membership.notes)) : ''
  ].join('');

  const rightColumn = [
    detailBlock('Plan', escapeHtml(plan.name)),
    detailBlock('Amount Paid', '&#8377;' + escapeHtml(amountPaid) + ' (' + billingCycleLabel(plan.billing_cycle).toLowerCase() + ')'),
    detailBlock('Payment Method', escapeHtml(membership.payment_method || 'razorpay')),
    detailBlock('Valid', formatDateNice(membership.start_date) + ' to ' + formatDateNice(membership.end_date))
  ].join('');

  const paymentIdsHtml = [
    detailBlock('Razorpay Order ID', escapeHtml(membership.razorpay_order_id || '-')),
    detailBlock('Razorpay Payment ID', escapeHtml(membership.razorpay_payment_id || '-')),
    detailBlock('Terms Accepted On', termsAcceptedLabel)
  ].join('');
  ...
  const html = [
    ...
    '<meta charset="UTF-8" />',
    '<meta name="viewport" content="width=device-width, initial-scale=1.0" />',
    '<title>New Membership Payment - PlayBox Kashmir™</title>',
    '<style>',
    "  body, table, td { font-family: 'Segoe UI', Arial, Helvetica, sans-serif; }",
    '  @media only screen and (max-width: 480px) {',
    '    .pk-container { width: 100% !important; }',
    ...
    '    <tr>',
    '      <td align="center">',
    '        <table role="presentation" class="pk-container" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(67,56,202,0.12);">',

    '          <tr>',
    '            <td align="center" style="background-color:#4338CA;padding:32px 24px 26px;">',
    '            <div style="font-size:20px;line-height:1.3;font-weight:700;color:#ffffff;">New Membership Payment</div>',
    '            <div style="font-size:13px;color:#e0e7ff;margin-top:4px;">register.playboxkashmir.com</div>',
    '            </td>',
    '          </tr>',

    '          <tr>',
    '            <td class="pk-body-pad" style="padding:28px 40px 8px;">',
    '            <p style="margin:0;font-size:14px;line-height:1.6;color:#4b5563;">A new member has just paid and their membership has been activated automatically. Details below:</p>',
    '            </td>',
    '          </tr>',

    '          <tr>',
    '            <td class="pk-body-pad" style="padding:16px 40px 4px;">',
    '              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">',
    '                <tr>',
    '                  <td class="pk-col" valign="top" width="50%" style="padding-right:16px;">' + leftColumn + '</td>',
    '                  <td class="pk-col" valign="top" width="50%">' + rightColumn + '</td>',
    ...
    '          <tr>',
    '            <td class="pk-body-pad" style="padding:8px 40px 0;">',
    '              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f9fafb;border-radius:8px;">',
    '                <tr><td style="padding:16px 20px;">' + paymentIdsHtml + '</td></tr>',
    '              </table>',
    '            </td>',
    '          </tr>',

    '          <tr>',
    '            <td class="pk-body-pad" style="padding:24px 40px 0;">',
    '              <p style="margin:0;font-size:12.5px;line-height:1.7;color:#6b7280;">This member can be viewed and managed from the <a href="https://www.playboxkashmir.com/admin/memberships.html" style="color:#0F766E;font-weight:600;text-decoration:none;">Memberships</a> section of the admin dashboard.</p>',
    '            </td>',
    '          </tr>',
    ...
    '              <p style="margin:0;font-size:12px;color:#9ca3af;text-align:center;">Automated notification from the PlayBox Kashmir™ membership system.</p>',
    '            </td>',
    '          </tr>',
