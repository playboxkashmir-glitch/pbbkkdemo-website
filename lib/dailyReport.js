...
function computeOpenRanges(openingHour, closingHour, blocks) {
  const openHours = [];
  for (let hour = openingHour; hour < closingHour; hour++) {
    const blocked = blocks.some((b) => isHourBlocked(hour, b));
    if (!blocked) openHours.push(hour);
  }
  const ranges = [];
  let i = 0;
  while (i < openHours.length) {
    let j = i;
    while (j + 1 < openHours.length && openHours[j + 1] === openHours[j] + 1) j++;
    ranges.push(formatRange(openHours[i], openHours[j] + 1));
    i = j + 1;
  }
  return ranges;
}
...
export async function runDailyReport() {
  ...
  const facilitySlots = facilities.map((f) => {
    const blocks = bookingsRes.rows
      .filter((b) => b.facility_id === f.id)
      .map((b) => ({ start_time: b.start_time, end_time: b.end_time }));
    return {
      facility_label: f.option_name,
      ranges: computeOpenRanges(openingHour, closingHour, blocks)
    };
  });
