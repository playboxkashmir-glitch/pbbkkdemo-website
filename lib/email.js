...
export async function sendBookingFailedEmail({ customer_name, customer_email, sport, booking_date, time_slot, suggestions }) {
    const fromAddress = process.env.SMTP_FROM || process.env.SMTP_USER;
    const t = getTransporter();
  ...
  const suggestionsHtml = (suggestions && suggestions.length)
      ? (
              ...
              '<ul style="margin:8px 0 0;padding-left:18px;color:#374151;font-size:13.5px;line-height:1.7;">' +
