...
// Sent when a customer submits the public membership sign-up form on
// register.playboxkashmir.com. This is a "request received" email, not a
// confirmation of an active membership - staff still need to collect
// payment and activate it from the admin Memberships page.
export async function sendMembershipSignupEmail(membership, plan) {
  const fromAddress = process.env.SMTP_FROM || process.env.SMTP_USER;
  const t = getTransporter();

  const subject = 'Membership Request Received - ' + plan.name + ' | PlayBox Kashmir';
  const memberName = escapeHtml(membership.member_name || 'there');
  const firstName = memberName.split(' ')[0];

  const leftColumn = [
    detailBlock('Plan', escapeHtml(plan.name)),
    detailBlock('Price', '&#8377;' + escapeHtml(plan.price) + ' / ' + billingCycleLabel(plan.billing_cycle).toLowerCase()),
  ...
  const rightColumn = [
    ...
    detailBlock('Status', '<span style="display:inline-block;background:#FEF3C7;color:#92400e;font-size:13px;font-weight:700;padding:5px 14px;border-radius:999px;">Pending Review</span>')
  ].join('');
  ...
  const html = [
    ...
    '<meta charset="UTF-8" />',
    '<meta name="viewport" content="width=device-width, initial-scale=1.0" />',
    '<title>Membership Request Received - PlayBox Kashmir™</title>',
    '<style>',
    "  body, table, td { font-family: 'Segoe UI', Arial, Helvetica, sans-serif; }",
    '  @media only screen and (max-width: 480px) {',
    '    .pk-container { width: 100% !important; }',
    ...
    '            <td align="center" style="background-color:#0F766E;padding:36px 24px 30px;">',
    '              <img src="' + LOGO_URL + '" alt="PlayBox Kashmir™" width="64" height="64" style="display:block;margin:0 auto 14px;border-radius:14px;" />',
    '            <div style="font-size:22px;line-height:1.3;font-weight:700;color:#ffffff;">Request Received! &#128077;</div>',
    '            </td>',
    '          </tr>',
    ...
    '            <td class="pk-body-pad" style="padding:32px 40px 8px;">',
    '              <p style="margin:0 0 6px;font-size:16px;color:#111827;">Hey ' + firstName + ',</p>',
    '            <p style="margin:0;font-size:14px;line-height:1.6;color:#4b5563;">Thanks for your interest in the <strong>' + escapeHtml(plan.name) + '</strong> membership at <strong>PlayBox Kashmir™</strong>. Our team will reach out shortly to collect payment and activate your plan. Here\'s what you submitted:</p>',
    '            </td>',
    '          </tr>',
    ...
    '                <tr>',
    '                  <td style="padding:18px 20px;">',
    '                    <div style="font-size:14px;font-weight:700;color:#92400e;margin-bottom:6px;">Nothing to do right now</div>',
    '                    <p style="margin:0;color:#78350f;font-size:13.5px;line-height:1.7;">Your membership is not active yet. We\'ll contact you on the phone number you provided to confirm payment, after which your plan benefits kick in.</p>',
    '                  </td>',
    '                </tr>',
    '              </table>',
    '            </td>',
